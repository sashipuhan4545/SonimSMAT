...
public class XP5S_SCOUT_Contact_Transfer_Util extends BaseUtil{
	...
	public void deleteExistingFiles() {
		...
		try {
			...
			if(!(value1 || value2 )) {
				...
				minWait();
				scrollText("Select");
				clickBtn(Locators_ContactTransfer.select_option);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_MENU);
				minWait();
				scrollText("Select all");
				clickBtn(Locators_ContactTransfer.Select_all_option);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_MENU);
				minWait();
				scrollText("Delete");
				clickBtn(Locators_ContactTransfer.delete_option);
				minWait();
				clickBtn(Locators_ContactTransfer.OK_Btn);
			}else {
				clickBackButton(4);
			}
		} catch (Exception e) {
		...
		try {
			if(isElementExist(appTitle)) {
				...
				if(appName.equals("SetUpWizard")) {
					clickBtn(Locators_ContactTransfer.HomeBtn_Wizard);
				}
	...
	public void Launch_Contact_Transfer() throws InterruptedException, IOException{
		...
		try {
			...
			customWait(3000);
			navigateApps(Locators_ContactTransfer.utilitiesTab,Locators_ContactTransfer.ContactTransferTitle);
			if(isElementExist(Locators_ContactTransfer.permission_pop_up)) {
				for(int i=0;i<3;i++) {
					minWait();
					clickBtn(Locators_ContactTransfer.allow_btn);
					minWait();
				}
			}
		//	aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_DOWN);
	...
	public void selectContactfileToImport() throws InterruptedException, IOException {
		...
		try {
			...
			//clickBtn(Locators_ContactTransfer.selectAllOptn);
			for(int i=0; i<=2;i++) {
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_UP);
				minWait();
			}minWait();
	...
	public void push_to_background_and_verify() throws InterruptedException, IOException{
		...
		try {
			...
			customWait(2000);
			validate_launchofApp(Locators_ContactTransfer.messaging_Title, "Messaging");
		} catch (org.openqa.selenium.NoSuchElementException e) {
			e.printStackTrace();
			sf.fail();
		}	
		...
		try {
			...
			if(!isElementExist(Locators_BaseUtil.noRecentApp)) {
				if(isElementExist(Locators_ContactTransfer.ContactTransferTitle)) {
					...
					scrollToElementWithDpadUp(Locators_ContactTransfer.ContactTransferTitle);
					aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_CENTER);
					customWait(2000);
					validateLaunchScoutApp(Locators_ContactTransfer.ContactTransferTitle, "Contact Transfer");
				}
	...
	public void validate_key_press() throws InterruptedException {
		...
		try {
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_HOME);
			minWait();
			if(isElementExist(Locators_ContactTransfer.messaging_Title)) {
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_APP_SWITCH);
				customWait(1500);
	...
	public void disableBluetooth() throws InterruptedException {
		 ...
		 */
		try {
			clickBtn(Locators_ContactTransfer.connectedDevices_oreo);
			minWait();
			clickBtn(Locators_ContactTransfer.Bluetooth);
			minWait();
			if (isElementExist(Locators_ContactTransfer.enabled)) {
				customWait(2000);
				clickBtn(Locators_ContactTransfer.enabled);
	...
	public void enable_BT_and_verify_from_CT() throws InterruptedException {
		...
		try {
			...
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_CENTER);
			minWait();
			if(isElementExist(Locators_ContactTransfer.searchingBT)) {
				check = true;
	...
	public void scan_for_BT_devices_verify() throws InterruptedException {
		...
		try {
			customWait(10000);
			if(isElementExist(Locators_ContactTransfer.scan_Button)) {
				clickBtn(Locators_ContactTransfer.scan_Button);
				minWait();
				if(isElementExist(Locators_ContactTransfer.searchingBT)) {
					check = true;
	...
	public void deny_and_verify() throws InterruptedException {
		...
		try {
			...
			OCR.my_main.validate_Using_OCR("Contact Transfer.png");
			customWait(2000);
			Boolean value1 = searchStringOCR("Contact Transfer wants to", "ocr");
			Boolean value2 = searchStringOCR("turn on Bluetooth", "ocr");
			if(value1 && value2) {
				clickDownButton(1);
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_CENTER);
				customWait(5000);
				...
				OCR.my_main.validate_Using_OCR("Contact Transfer.png");
				customWait(2000);
				Boolean value3 = searchStringOCR("Bluetoolh will need to be", "ocr");
				Boolean value4 = searchStringOCR("enabled to use the BT", "ocr");
	...
	public void validateSearchfield(String contactName) throws InterruptedException, IOException {
		...
		try {
			...
			OCR.my_main.validate_Using_OCR("Contact Transfer.png");
			customWait(2000);
			Boolean value1 = searchStringOCR("S0uthernL|NC", "ocr");
	...
	public void verify_Notification_bar_when_import_in_progress() throws InterruptedException, IOException {
		...
		try {
			...
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_MENU);
			minWait();
			clickBtn(Locators_ContactTransfer.myNotification);
			minWait();
			takeScreenShotForOcrPngFormat("Contact Transfer");
			Thread.sleep(2000);
			OCR.my_main.validate_Using_OCR("Contact Transfer.png");
	...
	public void verify_succes_icon_in_Notification_bar() throws InterruptedException, IOException {
		...
		try {
			...
			OCR.my_main.validate_Using_OCR("Contact Transfer.png");
			customWait(2000);
			Boolean value1 = searchStringOCR("Completed contact imp..", "ocr");
			if(value1) {
				check = true;
