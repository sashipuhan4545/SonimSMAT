...
public class XP5S_Sanity_Util extends BaseUtil {
...
//			System.out.println(get_Timezone);
	...
	public void selectMoreOptions() throws InterruptedException, IOException {
		/*
...
//		    System.out.println(fmt1); 
		...
		*/
		...
		public void validate_phone_Charging() throws InterruptedException{
			...
			try {
				minWait();
				if(isElementExist(Locators_Sanity.USB_debugging_connected)){
					APP_LOGS.info("USB Debugging connected option is displayed");
					check=true;
					Assert.assertTrue(check);
				}else{
					APP_LOGS.info("USB Debugging connected option NOT is displayed");
					Assert.fail();
				}	
	...
	/*	public void takePicture_and_validate() throws InterruptedException{
			...
			minWait();
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_CENTER);
			APP_LOGS.info("Camera clicked successfully.");
			minWait();
			try {
		...
		public void validate_HomeScreen() throws InterruptedException{
			...
			minWait();
			try {
				if(isElementExist(Locators_Sanity.settings)){
					APP_LOGS.info("Settings icon is found in home screen");
		...
		/*public void validate_music_player() throws InterruptedException{
			launch_an_app("music");
			try {
				if(validate_presenceOfElement(Locators_Sanity.artists_music_player, "Artists tab in Music player")){
					APP_LOGS.info("Music player is launched successfully");
					Assert.assertTrue(true);
				}else{
					APP_LOGS.info("Music player is not launched successfully");
					Assert.fail();
				}
			} catch (NoSuchElementException e) {
				// TODO Auto-generated catch block
