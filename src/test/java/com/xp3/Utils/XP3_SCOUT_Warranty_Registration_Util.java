...
public class XP3_SCOUT_Warranty_Registration_Util extends BaseUtil {
	...
	public void Launch_Warranty_Registration() throws InterruptedException, IOException{
		...
		try {
			...
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_RIGHT);
			customWait(3000);
			navigateApps(Locators_Warranty_Registration.supportTab,Locators_Warranty_Registration.WarrantyReg);
			if(isElementExist(Locators_Warranty_Registration.permission_pop_up)) {
				for(int i=0;i<3;i++) {
					minWait();
					clickBtn(Locators_Warranty_Registration.allow_btn);
					minWait();
				}
			}
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_DOWN);
		...
		try {
			if(isElementExist(appTitle)) {
				...
				if(appName.equals("SetUpWizard")) {
					clickBtn(Locators_Warranty_Registration.HomeBtn_Wizard);
	...
	public void deleteExistingFiles() {
		...
		try {
			...
			Thread.sleep(2000);
			enlargeImage("src/test/resources/OCR_FILES/WarrantyReg.png", "src/test/resources/OCR_FILES/WarrantyReg.png");
			minWait();
			...
			if(!value) {
				...
				minWait();
				scrollText("Select");
				clickBtn(Locators_Warranty_Registration.select_option);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_MENU);
				minWait();
				scrollText("Select all");
				clickBtn(Locators_Warranty_Registration.Select_all_option);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_MENU);
				minWait();
				scrollText("Delete");
				clickBtn(Locators_Warranty_Registration.delete_option);
				minWait();
				clickBtn(Locators_Warranty_Registration.SWR_OK_Btn);
			}else {
				clickBackButton(4);
			}
		} catch (Exception e) {
	...
	public void disableCellularData() throws InterruptedException {
		...
		try {
			...
			clickBtn(Locators_Warranty_Registration.networkAndInternet);
			minWait();
			if(isElementExist(Locators_Warranty_Registration.MobileNetwrks_oreo)) {
				minWait();
				clickBtn(Locators_Warranty_Registration.MobileNetwrks_oreo);
				APP_LOGS.info("Selected Mobile Networks");
				minWait();
				if(isElementExist(Locators_SonimCare.enabled_Data_Oreo)) {
					OFF_Switch("Mobile data");
				}
				//	disableFeature(Locators_Warranty_Registration.enabled_Data_Oreo,Locators_Warranty_Registration.disabled_Data_Oreo);
				minWait();
				clickBtn(Locators_Warranty_Registration.SWR_OK_Btn);
				minWait();
			}
			else {
				clickBtn(Locators_Warranty_Registration.dataUsageOptn);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_DOWN);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_DOWN);
				...
				//disableFeature(Locators_Warranty_Registration.cellular_DataON,Locators_Warranty_Registration.cellular_DataOFF);
				minWait();
				clickBtn(Locators_Warranty_Registration.SWR_OK_Btn);
				minWait();
			}
	...
	public void disablewifi_changeMode_enablewifi() throws InterruptedException, IOException {
		...
		try {
			...
			minWait();
			customWait(2000);
			disableFeature(Locators_Warranty_Registration.enabled,Locators_Warranty_Registration.disabled);
			minWait();
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_BACK);
			clickBackButton(2);
			clearStorage();
	...
	public void changeToTestMode() {
		...
		try {
			...
			clickDownButton(1);
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_CENTER);
			Locators_Warranty_Registration.url_Field.clear();
			customWait(2000);
			enterTextToInputField(Locators_Warranty_Registration.url_Field, "https://test.sonimcloud.com");
			customWait(2000);
			clickBtn(Locators_Warranty_Registration.SWR_OK_Btn);
			clickBackButton(2);
			customWait(5000);
	...
	public void clearStorage() {
		...
		try {
			launch_an_app("settings");
			clickBtn(Locators_Warranty_Registration.Apps_Notification);
			minWait();
			clickBtn(Locators_Warranty_Registration.Apps_info);
			minWait();
			Runtime.getRuntime().exec("adb -s "+ p_Id +" shell input text warranty");
			customWait(2000);
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_CENTER);
			minWait();
			clickBtn(Locators_Warranty_Registration.storage);
			minWait();
			clickBtn(Locators_Warranty_Registration.cleardata);
			customWait(3000);
			clickBackButton(5);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void ConnectSecuredWifi() throws InterruptedException, IOException {
		...
		try {
			...
			navigateToWifiOption();
			minWait();
			if(isElementExist(Locators_Warranty_Registration.disabled)) {
				enableFeature(Locators_Warranty_Registration.enabled,Locators_Warranty_Registration.disabled,Locators_Warranty_Registration.switch_Title);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_BACK);		
				customWait(3000);
			}		
			...
			customWait(3000);
			String getSummary = Locators_Warranty_Registration.summaryWIFI_oreo.getText();
			...
			if(getSummary.contains("Not connected")) {
				...
				//	selectIDcwifi();      
				minWait();
				if(isElementExist(Locators_Warranty_Registration.wifi_IDC_ForgetBtn)) {
					clickBtn(Locators_Warranty_Registration.wifi_IDC_ForgetBtn);
					minWait();
					scrollToText("IDCSONWAP");
					//selectIDcwifi();    
				}
				clickBtn(Locators_Warranty_Registration.wifi_show_Pswd);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_UP);
				customWait(1000);

				changeToNumberMode_SMS(Locators_Warranty_Registration.wifi_IDC_Psswd);
				customWait(9000);
				if(isElementExist(Locators_Warranty_Registration.wifi_IDC)) {
				...
				minWait();
				customWait(1000);
				String psswrd = Locators_Warranty_Registration.wifi_IDC_Psswd.getText();
				System.out.println(psswrd);
				clickBtn(Locators_Warranty_Registration.wifi_IDC_ConnectBtn);
	...
	public void validateNowOption() throws InterruptedException {
		...
		try {
			if(isElementExist(Locators_Warranty_Registration.Now_Button)) {
				minWait();
				clickBtn(Locators_Warranty_Registration.Now_Button);
				if(isElementExist(Locators_Warranty_Registration.manual_reg_String)) {
					check = true;
					customWait(1000);
	...
	public void validateLaterOption() throws InterruptedException {
		...
		try {
			System.out.println(Locators_Warranty_Registration.Later_Button.getText());
			minWait();
			if(isElementExist(Locators_Warranty_Registration.Later_Button)) {
				minWait();
				clickBtn(Locators_Warranty_Registration.Later_Button);
				customWait(2000);
				takeScreenShotForOcrPngFormat("WarrantyReg");
				Thread.sleep(2000);
				OCR.my_main.validate_Using_OCR("WarrantyReg.png");
				customWait(2000);
				Boolean value = searchStringOCR("Suppo", "ocr");
				if(value) {
					check = true;
					customWait(1000);
	...
	public void validateNeverOption() throws InterruptedException {
		...
		try {
			boolean check1 = validateNeverOptionwithYesorNoOption(Locators_Warranty_Registration.no_Btn, "No");
			minWait();
			boolean check2 = validateNeverOptionwithYesorNoOption(Locators_Warranty_Registration.yes_Btn, "Yes");
			customWait(2000);
			if(check1 && check2) {
				check = true;
				customWait(1000);
	...
	public boolean validateNeverOptionwithYesorNoOption(WebElement element,String option) throws InterruptedException {
		 ...
		 */
		try {
			if(isElementExist(Locators_Warranty_Registration.Never_Button)) {
				minWait();
				clickBtn(Locators_Warranty_Registration.Never_Button);
				if(isElementExist(Locators_Warranty_Registration.never_Optn_Confirmation_Popup)) {
					clickBtn(element);
					if(option == "Yes") {
						takeScreenShotForOcrPngFormat("WarrantyReg");
						Thread.sleep(2000);
					...
					Boolean value = searchStringOCR("Suppo", "ocr");
					minWait();
					if(isElementExist(Locators_Warranty_Registration.RegisterYourDeviceOptn)|| value ) {
						check = true;
					}else {
						check = false;
	...
	public boolean validateResetButtonNoOption() throws InterruptedException {
		 ...
		 */
		try {
			enterTextToInputField(Locators_Warranty_Registration.firstNameTextField, "AUTOMATION");
			minWait();
			scrollToElementWithDpadDown(Locators_Warranty_Registration.resetBtn);
			minWait();
			System.out.println(Locators_Warranty_Registration.resetBtn);
			clickBtn(Locators_Warranty_Registration.resetBtn);
			customWait(2000);
			if(isElementExist(Locators_Warranty_Registration.ResetConfirmationPopUp)) {
				clickBtn(Locators_Warranty_Registration.no_Btn);
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_LEFT);
				minWait();
				for(int i=0;i<19;i++) {
					aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_UP);
				...
				if(searchStringOCR("AUTOMATION", "ocr") || searchStringOCR("AUTO", "ocr") || searchStringOCR("AUTO MATI O N", "ocr")) {
					check = true;
					customWait(1000);
	...
	public boolean validateResetButtonYesOption() throws InterruptedException {
		...
		try {
			...
			clickBtn(Locators_Warranty_Registration.resetBtn);
			if(isElementExist(Locators_Warranty_Registration.ResetConfirmationPopUp)) {
				clickBtn(Locators_Warranty_Registration.yes_Btn);
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_LEFT);
				minWait();
				for(int i=0;i<19;i++) {
					aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_UP);
				}
				//	scrollToElementWithDpadUp(Locators_Warranty_Registration.PersonalDetailsString);
				clickDownButton(1);
				if(isElementExist(Locators_Warranty_Registration.firstNameTextField)) {
					check = true;
					customWait(1000);
	...
	public void validatePressenceOfUploadFiles() throws InterruptedException {
		...
		try {
			...
			minWait();
			if(isElementExist(Locators_Warranty_Registration.uploadFilesString) && isElementExist(Locators_Warranty_Registration.uploadFilesOption)) {
				check = true;
				customWait(1000);
	...
	public void validateTakeAPicture() throws InterruptedException {
		...
		try {
			...
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_UP);
			minWait();
			clickBtn(Locators_Warranty_Registration.uploadFilesOption);
			minWait();
			if(isElementExist(Locators_Warranty_Registration.take_a_picture_option)) {
				clickBtn(Locators_Warranty_Registration.take_a_picture_option);
				customWait(2000);
				/*	aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_UP);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_CENTER);*/
				clickBtn(Locators_Warranty_Registration.captureImageOption);
				customWait(5000);
				if(isElementExist(Locators_Warranty_Registration.addImagePopUp)) {
					minWait();
					System.out.println(Locators_Warranty_Registration.SWR_OK_Btn.getText());
					clickBtn(Locators_Warranty_Registration.SWR_OK_Btn);
					minWait();
					clickDownButton(1);
					if(isElementExist(Locators_Warranty_Registration.imageAddedOption)) {
						check = true;
						customWait(1000);
	...
	public void validateFileType() throws InterruptedException {
		...
		try {
			...
			//	scrollToText("Proof of Purchase");
			minWait();
			scrollToElementWithDpadDownRuntime(Locators_Warranty_Registration.submitBtn);
			customWait(2000);
			boolean check1 = selectandValidatefile(1,Locators_Warranty_Registration.jpegFile, "jpeg");
			customWait(2000);
			boolean check2 = selectandValidatefile(2, Locators_Warranty_Registration.jpgFile, "jpg");
			customWait(2000);
			boolean check3 = selectandValidatefile(3,Locators_Warranty_Registration.pngFile, "png");
			customWait(2000);
			boolean check4 = selectandValidatefile(4, Locators_Warranty_Registration.docFile, "doc");
			customWait(2000);
			boolean check5 = selectandValidatefile(5, Locators_Warranty_Registration.pdfFile, "pdf");
			customWait(2000);
	...
	public void validateInvalidFileType() throws InterruptedException {
		...
		try {
			...
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_CENTER);
			minWait();
			if(isElementExist(Locators_Warranty_Registration.invalidFileTypeErrorMsg)) {
				clickBtn(Locators_Warranty_Registration.SWR_OK_Btn);
				minWait();
				check = true;
	...
	public void NavigateToselectFile() {
		...
		try {
			...
			clickBtn(Locators_Warranty_Registration.uploadFilesOption);
			minWait();
			//	clickBtn(Locators_Warranty_Registration.chooseAFileOption);
			minWait();
			clickDownButton(2);
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_CENTER);
			clickDownButton(2);
	...
	public void validateEachMandatoryField(String firstname,String lastname,String country,String company,String state,String city,String zipCode,String address,String phoneNumber,String email_id) throws InterruptedException {
		...
		try {
			...
			//scrollToText("SUBMIT");
			scrollToElementWithDpadDown(Locators_Warranty_Registration.submitBtn);
			clickBtn(Locators_Warranty_Registration.submitBtn);
			validatemandatoryPopUp("First_Name Field");
			checkAndFillField(Locators_Warranty_Registration.firstNameTextField, firstname,"First_Name Field");

			scrollToElementWithDpadDown(Locators_Warranty_Registration.submitBtn);
			clickBtn(Locators_Warranty_Registration.submitBtn);
			validatemandatoryPopUp("Last_Name Field");
			checkAndFillField(Locators_Warranty_Registration.lastNameTextField, lastname,"Last_Name Field");
			minWait();

			scrollToElementWithDpadDown(Locators_Warranty_Registration.submitBtn);
			clickBtn(Locators_Warranty_Registration.submitBtn);
			validatemandatoryPopUp("Country Field");
			checkAndFillField(Locators_Warranty_Registration.countryTextField, country,"Country Field");

			scrollToElementWithDpadDown(Locators_Warranty_Registration.submitBtn);
			clickBtn(Locators_Warranty_Registration.submitBtn);
			validatemandatoryPopUp("Company_Name Field");
			checkAndFillField(Locators_Warranty_Registration.companyTextField, company , "Company_Name Field");

			scrollToElementWithDpadDown(Locators_Warranty_Registration.submitBtn);
			clickBtn(Locators_Warranty_Registration.submitBtn);
			validatemandatoryPopUp("State Field");
			checkAndFillField(Locators_Warranty_Registration.stateTextField, state,"State Field");

			scrollToElementWithDpadDown(Locators_Warranty_Registration.submitBtn);
			clickBtn(Locators_Warranty_Registration.submitBtn);
			validatemandatoryPopUp("City Field");
			checkAndFillField(Locators_Warranty_Registration.cityTextField, city ,"City Field");

			scrollToElementWithDpadDown(Locators_Warranty_Registration.submitBtn);
			clickBtn(Locators_Warranty_Registration.submitBtn);
			validatemandatoryPopUp("ZipCode Field");
			checkAndFillField(Locators_Warranty_Registration.zipcodeTextField, zipCode ,"ZipCode Field");

			scrollToElementWithDpadDown(Locators_Warranty_Registration.submitBtn);
			clickBtn(Locators_Warranty_Registration.submitBtn);
			validatemandatoryPopUp("address1 Field");
			checkAndFillField(Locators_Warranty_Registration.address1TextField, address ,"address1 Field");

			scrollToElementWithDpadDown(Locators_Warranty_Registration.submitBtn);
			clickBtn(Locators_Warranty_Registration.submitBtn);
			validatemandatoryPopUp("Phone_Number Field");
			checkAndFillField(Locators_Warranty_Registration.phoneNumberTextField, phoneNumber ,"Phone_Number Field");

			scrollToElementWithDpadDown(Locators_Warranty_Registration.submitBtn);
			clickBtn(Locators_Warranty_Registration.submitBtn);
			validatemandatoryPopUp("email_id Field");
			checkAndFillField(Locators_Warranty_Registration.email_idTextField, email_id ,"email_id Field");

			scrollToElementWithDpadDown(Locators_Warranty_Registration.submitBtn);
			clickBtn(Locators_Warranty_Registration.submitBtn);
			validatemandatoryPopUp("purchased_on Field");
			if(isElementExist(Locators_Warranty_Registration.selectPurchasedDateField)) {
				minWait();
				clickBtn(Locators_Warranty_Registration.selectPurchasedDateField);
				minWait();
				clickBtn(Locators_Warranty_Registration.choose_Btn);
			}else {
	...
	public void validatemandatoryPopUp(String Field) throws InterruptedException {
		 ...
		 */
		try {
			if(isElementExist(Locators_Warranty_Registration.mandatoryFieldPopUp)) {
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_UP);
				minWait();
				clickBtn(Locators_Warranty_Registration.SWR_OK_Btn);
				check = true;
	...
	public void getVersion() {
		...
		try {
			...
			//	clickBtn(Locators_Warranty_Registration.More_option);
			customWait(2000);
			clickBtn(Locators_Warranty_Registration.about_option);
			customWait(3000);
			if(isElementExist(Locators_Warranty_Registration.app_version)) {
				String version = Locators_Warranty_Registration.app_version.getText();
				check = true;
	...
	public void validateAbout() {
		...
		try {
			...
			customWait(2000);
			clickBtn(Locators_Warranty_Registration.about_option);
			boolean value1 = getTextAndValidate(Locators_Warranty_Registration.copyright_option, "Copyright ©2019");
			boolean value2 = getTextAndValidate(Locators_Warranty_Registration.copyright_option, "Sonim Technologies Inc.");
			boolean value3 = getTextAndValidate(Locators_Warranty_Registration.AllRightsReservedString, "All rights Reserved");
			boolean value4 = getTextAndValidate(Locators_Warranty_Registration.sonimPrivacyPolicyString, "Sonim Privacy Policy");
			boolean value5 = getTextAndValidate(Locators_Warranty_Registration.PrivacyPolicylink, "https://www.sonimtech.com/index.php/policy");
	...
	public void fillAllTheFields(String firstname,String lastname,String country,String company,String state,String city,String zipCode,String address1,String address2,String phoneNumber,String email_id,String dealerName) throws InterruptedException {
		...
		try {
			...
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_CENTER);
			minWait();
			scrollToElementWithDpadDown(Locators_Warranty_Registration.address1TextField);
			minWait();
			checkAndFillField(Locators_Warranty_Registration.companyTextField, company , "Company_Name Field");
			minWait();
			checkAndFillField(Locators_Warranty_Registration.stateTextField, state,"State Field");
			...
			checkAndFillField(Locators_Warranty_Registration.zipcodeTextField, zipCode ,"ZipCode Field");
			minWait();
			checkAndFillField(Locators_Warranty_Registration.address1TextField, address1 ,"address1 Field");
			minWait();
			scrollToElementWithDpadDown(Locators_Warranty_Registration.email_idTextField);
			checkAndFillField(Locators_Warranty_Registration.address2TextField, address2 ,"address2 Field");
			minWait();
			checkAndFillField(Locators_Warranty_Registration.phoneNumberTextField, phoneNumber ,"Phone_Number Field");
			minWait();
			checkAndFillField(Locators_Warranty_Registration.email_idTextField, email_id ,"email_id Field");
			...
			scrollToElementWithDpadDown(Locators_Warranty_Registration.selectPurchasedDateField);
			checkAndFillField(Locators_Warranty_Registration.dealer_Name, dealerName ,"dealer_Name Field");
			minWait();
			clickDownButton(1);
			System.out.println(Locators_Warranty_Registration.selectPurchasedDateField.getText());
			if(isElementExist(Locators_Warranty_Registration.selectPurchasedDateField)) {
				minWait();
				clickBtn(Locators_Warranty_Registration.selectPurchasedDateField);
				minWait();
	...
	public void validateSubmitOptionWithNoOption() {
		...
		try {
			...
			clickBtn(Locators_Warranty_Registration.submitBtn);
			minWait();
			if(isElementExist(Locators_Warranty_Registration.submit_pop_up)) {
				minWait();
				clickBtn(Locators_Warranty_Registration.no_Btn);
				minWait();
				takeScreenShotForOcrPngFormat("WarrantyReg");
	...
	public void validateSubmitOptionWithYesOption() {
		...
		try {
			...
			if(isElementExist(Locators_Warranty_Registration.submit_pop_up)) {
				...
				OCR.my_main.validate_Using_OCR("WarrantyReg.png");
				customWait(2000);
				Boolean value1 = searchStringOCR("Information", "ocr");
				minWait();
				Boolean value2 = searchStringOCR("Name", "ocr");
				if(value1 && value2) {
					check = true;
					customWait(1000);
	...
	public void validateRegisteredInfo() {
		...
		try {
			...
			OCR.my_main.validate_Using_OCR("WarrantyReg.png");
			customWait(2000);
			Boolean value = searchStringOCR("Information", "ocr");
			if(value) {
				Boolean value1 = compare("Test","Name","Automation Test");
				clickDownButton(2);
				customWait(2000);
				Boolean value2 = compare("Sonim","Company Name ","Sonim");
				customWait(2000);
				Boolean value3 = compare("560078","Address","JPNagar, Bangalore, Bangalore, Karnataka, India- 560078");
				customWait(2000);
				clickDownButton(2);
				Boolean value4 = compare("987654321","Phone Number","9876543210");
				customWait(2000);
				Boolean value5 = compare("@gmai|","Email ID","automationtest@gmail.com");
				customWait(2000);
				clickDownButton(2);
				minWait();
				Boolean value6 = compare("XP3800","Model No.","XP3800");
				customWait(2000);
				clickDownButton(2);
	...
	public void changeFontSize(WebElement element,boolean value) {
		/*
		...
		try {
			launch_an_app("settings");
			minWait();
			clickBtn(Locators_Warranty_Registration.display_option);
			minWait();
			clickBtn(Locators_Warranty_Registration.fontsize_option);
			minWait();
			if(value) {
				clickDownButton(17);
			}else {
