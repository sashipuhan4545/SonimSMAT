...
public class XP3_SCOUT_SonimCare_Util extends BaseUtil {
	...
	public void Launch_SonimCare() throws InterruptedException, IOException{
		...
		try {
			...
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_RIGHT);
			customWait(2000);
			navigateApps(Locators_SonimCare.supportTab,Locators_SonimCare.SonimCare);
			if(isElementExist(Locators_SonimCare.permission_pop_up)) {
				for(int i=0;i<3;i++) {
					minWait();
					clickBtn(Locators_SonimCare.allow_btn);
					minWait();
				}
			}
		...
		try {
			if(isElementExist(appTitle)) {
				...
				if(appName.equals("SetUpWizard")) {
					clickBtn(Locators_SonimCare.HomeBtn_Wizard);
				}
	...
	public void validatePressence_of_AllOptions() {
		...
		try {
			aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_DOWN);
			minWait();
			if(isElementExist(Locators_SonimCare.Self_diagnosis_Test)) {
				check1=true;
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_DOWN);
				minWait();
			} if(isElementExist(Locators_SonimCare.Battery_Diagnosis_Test)) {
				check2=true;
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_DOWN);
				minWait();
			} if(isElementExist(Locators_SonimCare.Diagnosis_Report)) {
				check3=true;
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_DOWN);
				minWait();
			} if(isElementExist(Locators_SonimCare.Videos)) {
				check4=true;
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_DOWN);
				minWait();
			} if(isElementExist(Locators_SonimCare.Online_Troubleshooting)) {
				check5=true;
				minWait();
			}
	...
	public void navigate_and_validate_self_diagnosis_welcome_screen() {
		...
		try {
			minWait();
			clickBtn(Locators_SonimCare.Self_diagnosis_Test);
			minWait();
			if(isElementExist(Locators_SonimCare.Self_diagnosis)) {
				if(isElementExist(Locators_SonimCare.Self_diagnosis_welcome_text) && isElementExist(Locators_SonimCare.Self_diagnosis_info_text)) {
					check = true ;
	...
	public void navigate_to_module(WebElement element,String text) {
		try {
			...
			clickBtn(Locators_SonimCare.Self_diagnosis_Test);
			minWait();
			clickBtn(Locators_SonimCare.Continue);
			minWait();
			clickBtn(Locators_SonimCare.Continue);
			minWait();
			scrollText(text);
	...
	public void testWifiModule_and_validate() {
		...
		try {
			minWait();
			if(isElementExist(Locators_SonimCare.module_welcome_page)) {
				clickBtn(Locators_SonimCare.Continue);
				minWait();
				clickBtn(Locators_SonimCare.Scan_for_more_networks);
				customWait(10000);
				...
				if(value) {
					select_yes_Btn();
					//	clickBtn(Locators_SonimCare.yes_Btn);
					if(isElementExist(Locators_SonimCare.Self_diagnosis)) {
						check = true ;
	...
	public void testBluetoothModule_and_validate() {
		...
		try {
			...
			if(isElementExist(Locators_SonimCare.module_welcome_page)) {
				clickBtn(Locators_SonimCare.Continue);
				minWait();
				//	clickBtn(Locators_SonimCare.Search_Bluetooth);
				aDriver.pressKeyCode(AndroidKeyCode.ENTER);
				customWait(10000);
	...
	public void test_BackCamera_Module_and_validate() {
		...
		try {
			...
			if(isElementExist(Locators_SonimCare.module_welcome_page)) {
				clickBtn(Locators_SonimCare.Continue);
				customWait(2000);
				//	clickBtn(Locators_SonimCare.capture_btn);
				aDriver.pressKeyCode(AndroidKeyCode.ENTER);
				minWait();
				if(isElementExist(Locators_SonimCare.add_image)) {
					clickBtn(Locators_SonimCare.ok_Btn);
					customWait(3000);
					if(isElementExist(Locators_SonimCare.captured_image)) {
						select_yes_Btn();
						if(isElementExist(Locators_SonimCare.Self_diagnosis)) {
							check = true ;
	...
	public void validate_add_image_cancel_functionality() {
		...
		try {
			...
			if(isElementExist(Locators_SonimCare.add_image)) {
				minWait();
				clickBtn(Locators_SonimCare.cancel_Btn);
				customWait(3000);
				if(isElementExist(Locators_SonimCare.module_welcome_page)) {
					check = true ;
	...
	public void test_flashlight_Module_and_validate() {
		...
		try {
			...
			if(isElementExist(Locators_SonimCare.module_welcome_page)) {
				clickBtn(Locators_SonimCare.Continue);
				minWait();
				clickBtn(Locators_SonimCare.flashlight_off_mode);
				customWait(5000);
				if(isElementExist(Locators_SonimCare.flashlight_on_mode)) {
					customWait(2000);
					clickBtn(Locators_SonimCare.flashlight_on_mode);
					customWait(3000);
					select_yes_Btn();
					if(isElementExist(Locators_SonimCare.Self_diagnosis)) {
						check = true ;
	...
	public void test_display_Module_and_validate() {
		...
		try {
			...
			if(isElementExist(Locators_SonimCare.module_welcome_page)) {
				clickBtn(Locators_SonimCare.Continue);
				minWait();
				if(isElementExist(Locators_SonimCare.display_info_page)) {
					for(int i=0;i<6;i++) {
						aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_BACK);
						minWait();
					}
					if(isElementExist(Locators_SonimCare.display_confirmation_msg)) {
						customWait(3000);
						select_yes_Btn();
						if(isElementExist(Locators_SonimCare.Self_diagnosis)) {
							check = true ;
	...
	public void test_vibration_Module_and_validate() {
		...
		try {
			...
			if(isElementExist(Locators_SonimCare.module_welcome_page)) {
				clickBtn(Locators_SonimCare.Continue);
				minWait();
				clickBtn(Locators_SonimCare.start_vibrate);
				customWait(2000);
				if(isElementExist(Locators_SonimCare.stop_vibrate)) {
					clickBtn(Locators_SonimCare.stop_vibrate);
					customWait(3000);
					select_yes_Btn();
					if(isElementExist(Locators_SonimCare.Self_diagnosis)) {
						check = true ;
	...
	public void check_self_diagnosis_pop_up() {
		try {
			if(isElementExist(Locators_SonimCare.self_diagnosis_pop_up)) {
				clickBtn(Locators_SonimCare.no_button);
				clickBackButton(2);
			}else {
				clickBackButton(3);
			}
	...
	public void test_clear_cache_memory_Module_and_validate() {
		...
		try {
			...
			if(isElementExist(Locators_SonimCare.module_welcome_page)) {
				...
				minWait();
				clickBtn(Locators_SonimCare.cached_data);
				minWait();
				if(isElementExist(Locators_SonimCare.clear_cached_data)) {
					minWait();
					clickBtn(Locators_SonimCare.ok_Btn);
					customWait(5000);
					if(isElementExist(Locators_SonimCare.cleared_cached_data)) {
						clickBackButton(1);
						if(isElementExist(Locators_SonimCare.Self_diagnosis)) {
							check = true ;
	...
	public void test_Storage_Info_Module_and_validate() {
		...
		try {
			...
			if(isElementExist(Locators_SonimCare.module_welcome_page)) {
				clickBtn(Locators_SonimCare.Continue);
				customWait(5000);
				if(isElementExist(Locators_SonimCare.storage_info_progress)) {
					customWait(2000);
					clickBackButton(1);
					if(isElementExist(Locators_SonimCare.Self_diagnosis)) {
						check = true ;
	...
	public void validate_wifi_no_option() {
		...
		try {
			...
			if(isElementExist(Locators_SonimCare.module_welcome_page)) {
				...
				if(value) {
					select_no_Btn();
					if(isElementExist(Locators_SonimCare.no_option_confirmation_pop_up)) {
						clickBtn(Locators_SonimCare.ok_Btn);
						check = true ;
	...
	public void navigate_and_validate_battery_diagnosis_welcome_screen() {
		...
		try {
			minWait();
			clickBtn(Locators_SonimCare.Battery_Diagnosis_Test);
			minWait();
			if(isElementExist(Locators_SonimCare.Battery_Diagnosis_Test)) {
				if(isElementExist(Locators_SonimCare.battery_diagnosis_welcome_screen) && isElementExist(Locators_SonimCare.battery_diagnosis_info_text)) {
					check = true ;
	...
	public void validate_battery_information() {
		...
		try {
			...
			clickBtn(Locators_SonimCare.Continue);
			minWait();
			if(isElementExist(Locators_SonimCare.Battery_information) && isElementExist(Locators_SonimCare.Battery_progress_bar) && isElementExist(Locators_SonimCare.Battery_health) && isElementExist(Locators_SonimCare.Battery_voltage) && isElementExist(Locators_SonimCare.Battery_temperature) && isElementExist(Locators_SonimCare.Battery_service_history)) {
				check = true ;
	...
	public void validateBatteryServiceHistory() {
		...
		try {
			...
			minWait();
			if(isElementExist(Locators_SonimCare.month_in_battery_diagnosis) && isElementExist(Locators_SonimCare.calendar_in_battery_diagnosis)) {
				String total_charges = Locators_SonimCare.Total_no_of_charges.getText();
				String charge_cycle = Locators_SonimCare.charge_cycle.getText();
				String max_temp_string = Locators_SonimCare.Max_temp_recorded.getText();
				String recorded_temp = Locators_SonimCare.temp_record.getText();
				String total_charge_duration = Locators_SonimCare.Total_Charge_Duration.getText();
				String charge_duration = Locators_SonimCare.charge_duration.getText();
	...
	public void validate_diagnosis_report() {
		SoftAssert SA = new SoftAssert();
		try {
			clickBtn(Locators_SonimCare.Diagnosis_Report);
			minWait();
			if(isElementExist(Locators_SonimCare.Diagnosis_Report) && isElementExist(Locators_SonimCare.diagnosis_report_list)) {
	...
	public boolean validate_diagnosis_report_no_option() {
		...
		try {
			...
			minWait();
			if(isElementExist(Locators_SonimCare.SonimCare)) {
				check = true ;
	...
	public void disableCellularData() throws InterruptedException {
		...
		try {
			...
			clickBtn(Locators_SonimCare.networkAndInternet);
			minWait();
			if(isElementExist(Locators_SonimCare.MobileNetwrks_oreo)) {
				minWait();
				clickBtn(Locators_SonimCare.MobileNetwrks_oreo);
				APP_LOGS.info("Selected Mobile Networks");
				minWait();
				if(isElementExist(Locators_SonimCare.enabled_Data_Oreo)) {
					OFF_Switch("Mobile data");
				}
			//	disableFeature(Locators_SonimCare.enabled_Data_Oreo,Locators_SonimCare.disabled_Data_Oreo);
				minWait();
				clickBtn(Locators_SonimCare.OK_Btn);
				minWait();
			}
			else {
				clickBtn(Locators_SonimCare.dataUsageOptn);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_DOWN);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_DOWN);
			...
			//	disableFeature(Locators_SonimCare.cellular_DataON,Locators_SonimCare.cellular_DataOFF);
				minWait();
				clickBtn(Locators_SonimCare.OK_Btn);
				minWait();
			}
	...
	public void ConnectSecuredWifi() throws InterruptedException, IOException {
		...
		try {
			...
			minWait();
			if(isElementExist(Locators_SonimCare.disabled)) {
				enableFeature(Locators_SonimCare.enabled,Locators_SonimCare.disabled,Locators_SonimCare.switch_Title);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_BACK);		
				customWait(3000);
			}		
			...
			customWait(3000);
			String getSummary = Locators_SonimCare.summaryWIFI_oreo.getText();
			...
			if(getSummary.contains("Not connected")) {
				...
				//	selectIDcwifi();      
				minWait();
				if(isElementExist(Locators_SonimCare.wifi_IDC_ForgetBtn)) {
					clickBtn(Locators_SonimCare.wifi_IDC_ForgetBtn);
					minWait();
					scrollToText("IDCSONWAP");
					//selectIDcwifi();    
				}
				clickBtn(Locators_SonimCare.wifi_show_Pswd);
				minWait();
				aDriver.pressKeyCode(AndroidKeyCode.KEYCODE_DPAD_UP);
				customWait(1000);

				changeToNumberMode_SMS(Locators_SonimCare.wifi_IDC_Psswd);
				customWait(9000);
				if(isElementExist(Locators_SonimCare.wifi_IDC)) {
				...
				minWait();
				customWait(1000);
				String psswrd = Locators_SonimCare.wifi_IDC_Psswd.getText();
				System.out.println(psswrd);
				clickBtn(Locators_SonimCare.wifi_IDC_ConnectBtn);
